...
switch(_idx)do
{
  case 1:{
           ...
           {
             _dir = [(position _streetblockname),(position (_next_streetsblock select 0))] call FNC_BIS(dirTo);
   ...
   case 2:{
            If(count _next_streetsblock > 0)then
            {
              ...
              _check_arr = [];
              {
                _chk_dir = [(position _streetblockname),(position _x)] call FNC_BIS(dirTo);
              ...
              If(count _check_arr > 0)then{_dir = (_check_arr select 0);}else{_dir = [(position _streetblockname),_optpos] call FNC_BIS(dirTo);}
            }else{_dir = [(position _streetblockname),_optpos] call FNC_BIS(dirTo);};
            _output = [(position _streetblockname),_dir];
          };
   case 3:{
            _dir = [(position _streetblockname),_optpos] call FNC_BIS(dirTo);
            _output = [(position _streetblockname),_dir];
          };
